...
{
	if(env->getExtensions()->elasticGC.elasticEnabled == 1)
	{
		if(_maximumSize > _currentSize + (_currentSize - env->getExtensions()->elasticGC.heapSize))
		{
		return _currentSize + (_currentSize - env->getExtensions()->elasticGC.heapSize);
		}
	}
...
MM_MemorySubSpace::maxContraction(MM_EnvironmentBase* env)
{
	if(env->getExtensions()->elasticGC.elasticEnabled == 1)
	{
		return _currentSize - env->getExtensions()->elasticGC.heapSize;
	}
